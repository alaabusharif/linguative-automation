...
issue, proposes a HubSpot Company + Deal for each genuinely new candidate,
and creates them only after a human approves — it never creates records
silently. It does not set "Match Type" and does not draft a proposal;
those stay separate, later, human-gated steps.
...
4. For each source entry in an unprocessed comment:
   ...
   c. Show the proposal and get explicit approval before creating anything
   ...
   d. Whether approved, declined, or skipped as a duplicate, note the
